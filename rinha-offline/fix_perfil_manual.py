...
# Usage of GaloCard in map
# find: onEquipar={() => setGaloAtivo(idx)}
# and replace with: onEquipar={() => setGaloAtivo(idx)}\n              onOpenEvolucao={setModalEvolucao}\n              onOpenRebirth={setModalRebirth}
if 'onOpenEvolucao={setModalEvolucao}' not in text:
    text = text.replace('onEquipar={() => setGaloAtivo(idx)}', 'onEquipar={() => setGaloAtivo(idx)}\n              onOpenEvolucao={setModalEvolucao}\n              onOpenRebirth={setModalRebirth}')

# End of Perfil component
# Find:
#       </div>
#     </div>
#   );
...
'''
    text = re.sub(r'(</div>\s*</div>\s*\);\s*\})', modals + r'\n\1', text)

with codecs.open('src/components/Perfil.tsx', 'w', 'utf-8') as f:
    f.write(text)
print("Done final manual fix")
