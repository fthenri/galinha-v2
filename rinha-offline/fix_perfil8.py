...
# Find the end of `Perfil` component to insert modals correctly.
# The `Perfil` component ends with:
#         </div>
#       </div>
#     );
#   }
# We should insert the modals before the final `</div>\n    </div>\n  );\n}`
text = re.sub(r'(</div>\n\s*</div>\n\s*\);\n\})', modals + r'\n\1', text)

# Move React useState to the top
text = text.replace('import { useState } from "react";\n\n', '')
text = 'import { useState } from "react";\n' + text

with codecs.open('src/components/Perfil.tsx', 'w', 'utf-8') as f:
    f.write(text)
print("Done Complete Modal Override")
