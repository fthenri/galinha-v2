...
export default function ModoTrial({ jogador }: { jogador: any }) {
    const galos = useJogadorStore(s => s.galos);
    const galoAtivoIndex = useJogadorStore(s => s.galoAtivoIndex);
    const meuGalo = jogador || (galoAtivoIndex >= 0 && galoAtivoIndex < galos.length ? galos[galoAtivoIndex] : null);
    ...
    const iniciarLoop = async () => {
        ...
        meuGalo.hp_max = Math.floor(hpBaseOriginal * (1 + bonusTrial.vida) * (1 + (itemMods.vidaBonus || 0)));
        ...
        while (isMounted.current) {
            ...
            novoInimigo.hp_atual = Math.floor(hpInimigoComRebirth * (1 - (itemMods.voodoo || 0)));
            ...
            while (meuGalo.hp_atual > 0 && novoInimigo.hp_atual > 0) {
                ...
                const atacante: Galo = turnoJogador ? meuGalo : novoInimigo;
                const defensor: Galo = turnoJogador ? novoInimigo : meuGalo;
                ...
                if (podeAtacar) {
                    ...
                    if (atacante === meuGalo) {
                        danoAtaque = Math.floor(danoAtaque * (1 + bonusTrial.dano) * (1 + (itemMods.ataqueBonus || 0)));
                    }
                    ...
                    if (foiRefletido) {
                        ...
                        danoReal = alvoDano.sofrerDano(alvoDano === meuGalo ? Math.floor(danoAtaque * (1 - (itemMods.bloqueioDano || 0))) : danoAtaque, defensor.tipo);
                        ...
                        const tinhaEscudo = (alvoDano.efeitos["Shield"] || 0) > 0;
                        danoReal = alvoDano.sofrerDano(alvoDano === meuGalo ? Math.floor(danoAtaque * (1 - (itemMods.bloqueioDano || 0))) : danoAtaque, atacante.tipo);
            ...
            if (meuGalo.hp_atual > 0) {
                if (true) {
                    ...
                    adicionarLog(`Trial Concluída! Bônus da classe ${meuGalo.nome} aumentado.`);
                    forceUpdate();
    ...
    return (
        <div className="p-6 flex flex-col items-center max-w-4xl mx-auto w-full">
            ...
            <h2 className="text-3xl font-bold mb-6">Trial</h2>
